...
import { Clock, ArrowRight, FileText, Loader2, CheckCircle2 } from 'lucide-react';
...
export function ProjectHistory({ onSelect }: { onSelect?: (item: HistoryItem) => void }) {
    const { history: specs, isLoading: loading, error, fetchHistory } = usePlanStore();
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    {specs.map((spec) => (
                        ...
                        >
                            ...
                            <div className="flex items-center text-xs font-medium text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity transform translate-x-[-10px] group-hover:translate-x-0">
